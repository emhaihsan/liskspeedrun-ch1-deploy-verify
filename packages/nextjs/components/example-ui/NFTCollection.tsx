...
export const NFTCollection = () => {
  ...
  const { data: nftName, isLoading: nameLoading } = useScaffoldContractRead({
    contractName: "MyNFT",
    functionName: "name",
  });

  const { data: nftSymbol, isLoading: symbolLoading } = useScaffoldContractRead({
    contractName: "MyNFT",
    functionName: "symbol",
  });

  const { data: totalSupply, isLoading: supplyLoading } = useScaffoldContractRead({
    contractName: "MyNFT",
    ...
    return (
      <div className="card w-96 bg-base-100 shadow-xl">
        <div className="card-body flex items-center justify-center">
          ...
          <p className="mt-4">Loading NFT data...</p>
        </div>
      </div>
    );
  }
